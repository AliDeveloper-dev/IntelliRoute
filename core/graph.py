...
# Define Graph class here to avoid circular imports
class Graph:
    def __init__(self):
        self.adj = {}
        self.coords = {}
    ...
    def add_node(self, node_id, lat=None, lon=None, name=None):
    ...
    def add_edge(self, u, v, weight=1.0, bidirectional=True):
    ...
    def distance(self, u, v):
        return self.haversine(self.coords[u], self.coords[v])
    ...
    def load_graph(self):
        try:
            ...
            for node_id, lat, lng, name in nodes_data:
                self.G.add_node(node_id, lat, lng, name)
    ...
    if viz.G:
        for node_id in viz.G.adj.keys():
            if node_id in viz.G.coords:
                ...
                node_name = viz.G.node_names.get(node_id, f'Location {node_id}')
    ...
    if path:
        ...
        for i in range(len(path) - 1):
            ...
            if edge_weight is not None:
                ...
                u_name = viz.G.node_names.get(u, f'Node {u}')
                v_name = viz.G.node_names.get(v, f'Node {v}')
        ...
        car_speed = CAR_TYPES[car_type]['speed']
        estimated_time = (total_distance / 1000) / car_speed * 60
        
        for node_id in path:
            if node_id in viz.G.coords:
                lat, lng = viz.G.coords[node_id]
                node_name = viz.G.node_names.get(node_id, f'Node {node_id}')
                route_coords.append({
                    'lat': lat,
                    'lng': lng,
                    'name': node_name
        ...
        return jsonify({
            ...
            'summary': {
                ...
                'num_steps': len(steps),
                'start_name': viz.G.node_names.get(source, f'Node {source}'),
                'end_name': viz.G.node_names.get(target, f'Node {target}')
            }
        })
